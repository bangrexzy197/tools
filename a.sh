...
MONITOR_FILE="/usr/local/bin/monitor.sh"
...
nohup bash $MONITOR_FILE > $LOG_FILE 2>&1 &

sleep 2

if pgrep -f "$MONITOR_FILE" >/dev/null; then
  echo "[✓] MONITOR BERHASIL DIAKTIFKAN"
  echo "[✓] LOG: $LOG_FILE"
else
  echo "[✗] MONITOR GAGAL AKTIF"
fi
